...
// Imported, aggregate-only history from Meta Events Manager. Meta's export
// reports how many Lead signals arrived from the browser and server, but it
// intentionally does not expose the people behind those historical events.
// Keeping the export separate from the per-prospect delivery ledger lets the
// admin dashboard reconcile totals without claiming an unprovable identity
// match.
export const metaHistoricalSnapshots = pgTable("meta_historical_snapshots", {
  id: varchar("id", { length: 200 }).primaryKey(),
  datasetId: varchar("dataset_id", { length: 32 }).notNull(),
  eventName: varchar("event_name", { length: 80 }).notNull(),
  rangeStartUnix: integer("range_start_unix").notNull(),
  rangeEndUnix: integer("range_end_unix").notNull(),
  processedCount: integer("processed_count").notNull(),
  browserReceivedCount: integer("browser_received_count").notNull(),
  serverReceivedCount: integer("server_received_count").notNull(),
  totalReceivedCount: integer("total_received_count").notNull(),
  deduplicatedCount: integer("deduplicated_count").notNull(),
  eventMatchQuality: decimal("event_match_quality", { precision: 4, scale: 2 }),
  advancedMatchingRate: integer("advanced_matching_rate"),
  affectedAdSpend: decimal("affected_ad_spend", { precision: 12, scale: 2 }),
  source: text("source").notNull().default("events_manager_csv"),
  importedAt: timestamp("imported_at").defaultNow().notNull(),
});

export const metaHistoricalEvents = pgTable("meta_historical_events", {
  id: varchar("id", { length: 220 }).primaryKey(),
  datasetId: varchar("dataset_id", { length: 32 }).notNull(),
  eventName: varchar("event_name", { length: 80 }).notNull(),
  unixTimeStart: integer("unix_time_start").notNull(),
  browserReceivedCount: integer("browser_received_count").notNull(),
  serverReceivedCount: integer("server_received_count").notNull(),
  totalReceivedCount: integer("total_received_count").notNull(),
  importedAt: timestamp("imported_at").defaultNow().notNull(),
...
export type MetaHistoricalSnapshot =
  typeof metaHistoricalSnapshots.$inferSelect;
export type MetaHistoricalEvent =
  typeof metaHistoricalEvents.$inferSelect;
