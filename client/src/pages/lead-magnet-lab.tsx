...
export default function LeadMagnetLab() {
  ...
  const routerUrl = `${origin}/experiments/lead-magnets/assign?experiment=jtbd`;
    ...
    <div className="min-h-screen bg-[#f5f7f3] text-slate-900">
      <header className="border-b border-white/10 bg-[#0d1510] text-white">
        <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
          <div className="flex items-center justify-between gap-5"><div className="flex items-center gap-3"><img src={logoImage} alt="PestFlow" className="h-12 w-auto" /><div><p className="text-sm font-black">PestFlow Lead-Magnet Lab</p><p className="text-[10px] uppercase tracking-[.17em] text-white/35">Internal experiment workspace</p></div></div><span className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-emerald-300">3 core · 1 niche</span></div>
          <div className="mt-12 grid gap-8 lg:grid-cols-[1fr_.8fr] lg:items-end"><div><div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-[10px] font-black uppercase tracking-[.16em] text-white/60"><FlaskConical className="h-3.5 w-3.5" />Experiment control center</div><h1 className="mt-5 max-w-4xl text-5xl font-black leading-[.95] tracking-[-.05em] sm:text-7xl">Start with the customer’s job to be done.</h1><p className="mt-6 max-w-2xl text-sm leading-7 text-white/50">The recommended test asks what the owner would fix first. Lead magnets remain available for targeted campaigns, not an equal traffic split.</p></div><div className="rounded-2xl border border-white/10 bg-white/5 p-5"><div className="flex items-center gap-2 text-emerald-300"><Shuffle className="h-4 w-4" /><p className="text-[10px] font-black uppercase tracking-[.16em]">Recommended JTBD path</p></div><p className="mt-3 break-all text-xs leading-5 text-white/55">{routerUrl}</p><div className="mt-4 grid grid-cols-2 gap-2"><CopyButton value={routerUrl} /><a href={`${routerUrl}&internal=1&popup-check=1`} className="flex h-9 items-center justify-center gap-2 rounded-lg bg-emerald-500 px-3 text-[10px] font-black text-white">Open JTBD flow<ArrowRight className="h-3.5 w-3.5" /></a></div></div></div>
        </div>
      </header>
            ...
            return (
              <article key={magnet.id} className="overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-sm">
                <div className="p-6 sm:p-7">
                  <div className="flex items-start justify-between gap-4"><span className="grid h-12 w-12 place-items-center rounded-2xl" style={{ background: magnet.accentSoft, color: magnet.accent }}><Icon className="h-6 w-6" /></span><span className="rounded-full border border-slate-200 px-3 py-1 text-[9px] font-black uppercase tracking-wider text-slate-400">{magnet.id === "bid-radar" ? "Niche · test later" : "Targeted campaign"}</span></div>
