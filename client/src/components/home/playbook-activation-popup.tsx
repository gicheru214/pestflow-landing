...
const GOOGLE_CLIENT_ID =
  "65383864801-kd754q4cjeep88638fus0e48kib9s4ts.apps.googleusercontent.com";
...
export function PlaybookActivationPopup() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      || localStorage.getItem(LEGACY_POPUP_SUBMITTED_KEY)
    ) {
      return;
    }
    if (
            ...
            {step === "playbook" ? (
              ...
              >
                ...
                <div className="mt-3 w-full border-t border-white/10 pt-3">
                  <p className="mb-2 text-center text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">
                    Returning user?
                  </p>
                  <button
                    type="button"
                    ...
                    className="mb-2 flex h-9 w-full items-center justify-center gap-2.5 rounded-lg bg-white text-[13px] font-medium text-gray-800 transition-colors hover:bg-gray-100 disabled:opacity-60"
                  >
                    {googleLoading ? (
                  ...
                  {googleErr && (
                    <p className="mb-1 text-center text-[11px] text-red-400">
                      {googleErr}
                    </p>
                  )}
                  <a
                    href="https://app.pestflow.org/login"
                    className="block text-center text-[11px] text-slate-500 transition-colors hover:text-slate-300"
                  >
