    ...
    <div className="grid md:grid-cols-2 gap-8 items-start">
        ...
        <div className="space-y-6">
          <div>
            <div className="flex justify-between text-sm font-semibold text-slate-700 mb-3">
              ...
              <span className="text-emerald-600 text-lg font-extrabold">{stops}</span>
            </div>
            <input
          ...
          <div>
            <div className="flex justify-between text-sm font-semibold text-slate-700 mb-3">
              ...
              <span className="text-emerald-600 text-lg font-extrabold">{trucks}</span>
            </div>
            <input
      ...
      <div className="flex flex-col gap-4">
        <AnimatePresence mode="wait">
          {gap > 0 ? (
            <motion.div
              ...
              exit={{ opacity: 0, y: -6 }}
              className="bg-red-50 border-2 border-red-100 rounded-2xl p-6 text-center"
            >
              <p className="text-xs text-red-400 font-bold uppercase tracking-widest mb-2">Estimated Annual Leak</p>
              <p className="text-5xl font-extrabold text-red-600 tracking-tight">${annualLeak.toLocaleString()}</p>
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -6 }}
              className="bg-emerald-50 border-2 border-emerald-100 rounded-2xl p-6 text-center"
            >
        ...
        <Link href="/onboarding">
          <Button className="w-full h-12 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-base rounded-xl">
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b-2 border-slate-100">
            ...
            <th className="text-center text-xs font-bold text-slate-400 uppercase tracking-wider pb-3 px-4">PestPac</th>
            <th className="text-center text-xs font-bold text-slate-400 uppercase tracking-wider pb-3 px-4">GorillaDesk</th>
          </tr>
        </thead>
        <tbody>
          {COMPARISON_ROWS.map((row, i) => (
            ...
            >
              <td className="py-3 pl-2 text-slate-800 text-sm font-medium">{row.feature}</td>
              <td className="py-3 px-4"><Cell val={row.pestflow} /></td>
              <td className="py-3 px-4"><Cell val={row.pestpac} /></td>
              <td className="py-3 px-4"><Cell val={row.gorilla} /></td>
            </tr>
          ))}
        </tbody>
      </table>
          ...
          {!submitted ? (
            <motion.div key="questions" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              ...
              <Button
                ...
                className="w-full h-12 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-base rounded-xl disabled:opacity-40 mt-2"
              >
        ...
        <AnimatePresence mode="wait">
          {submitted ? (
            ...
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="rounded-2xl border-2 border-dashed border-slate-200 p-8 text-center flex flex-col items-center justify-center min-h-[280px]"
            >
              <div className="h-14 w-14 rounded-2xl bg-slate-100 flex items-center justify-center mb-4">
                <Zap className="h-7 w-7 text-slate-400" />
              </div>
              <p className="text-slate-400 font-medium text-sm">Answer the 3 questions<br />to see your Route IQ score</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
    ...
    <section className="py-20 bg-slate-50">
      <div className="container mx-auto px-4 md:px-6 max-w-6xl">
        <div className="text-center mb-14">
          ...
          <h2 className="text-3xl md:text-5xl font-extrabold text-slate-900 font-heading tracking-tight mb-4">
