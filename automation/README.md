...
This repo publishes one original PestFlow article per day from `automation/pestflow-blog-topics.json`. Prewritten articles can also carry a future `publishedAt` timestamp; the blog, direct route, metadata, and sitemap expose them automatically when that timestamp arrives.
...
2. The current prewritten backlog publishes one article per day through August 9, 2026. Future-dated articles remain absent from the blog, direct routes, metadata, and sitemap until their `publishedAt` time.
3. While that backlog exists, `scripts/generate-pestflow-blog.mjs` skips new generation so the site does not accidentally publish twice in one day. After the backlog ends, it picks the next unpublished long-tail topic, pulls recent Google News RSS context, asks OpenAI for JSON article content, strips em and en dashes, and writes to `client/src/content/generated-blog-posts.json`.
